<template>
  <div class="general-tab">
    <div class="form-section">
      ...
      <div class="form-row">
        <label for="code">URL Slug *</label>
        <div class="slug-input-wrapper">
          <span class="slug-prefix">/editathon/</span>
          <input
            id="code"
            v-model="localData.code"
            type="text"
            class="form-input slug-input"
            placeholder="wiki-loves-village-2025"
            @input="onSlugInput"
          />
        </div>
        <div v-if="slugError" class="field-error">{{ slugError }}</div>
        <div v-else class="field-hint">Lowercase letters, numbers, hyphens (-) and underscores (_) only. Used in the URL.</div>
      </div>
...
function onTitleInput() {
  // Auto-generate slug from title only if slug is still empty or was auto-generated
  if (!localData.code || localData._slugAutoGenerated) {
    localData.code = slugifyTitle(localData.title)
    localData._slugAutoGenerated = true
    slugError.value = ''
  }
  markUnsaved()
}
...
function onSlugInput() {
  const val = localData.code
  if (val && !/^[a-z0-9][a-z0-9_-]*$/.test(val)) {
    slugError.value = 'Only lowercase letters, numbers, hyphens (-) and underscores (_) allowed'
  } else {
    slugError.value = ''
  }
  localData._slugAutoGenerated = false  // user took manual control
  markUnsaved()
}
